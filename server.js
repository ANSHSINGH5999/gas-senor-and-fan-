...
let sensorData   = { temp:0, gas:0, fan:false, alarm:false, mode:'auto', updatedAt:null };
let controlState = { mode:'auto', fan:false, mute:false, updatedAt:null };
...
const isOnline = () => sensorData.updatedAt !== null && (Date.now() - new Date(sensorData.updatedAt).getTime()) < CONFIG.ESP32_TIMEOUT_MS;
const dataAge  = () => sensorData.updatedAt ? Math.floor((Date.now() - new Date(sensorData.updatedAt).getTime()) / 1000) : null;

function pushToBuffer(buffer, entry, max) { buffer.push(entry); if (buffer.length > max) buffer.shift(); }
...
function checkAndLogAlert(data) {
  ...
  if (reasons.length > 0) pushToBuffer(alerts, { ts: new Date().toISOString(), reasons, temp: data.temp, gas: data.gas }, CONFIG.MAX_ALERTS);
}
...
app.get('/', (_req, res) => {
  res.sendFile(path.join(__dirname, 'public', 'index.html'), (err) => {
    if (err) res.json({ service:'AEGIS Sentinel Backend', version:'4.0.0', status:'operational', esp32: isOnline() ? 'ONLINE':'OFFLINE' });
  });
});
...
app.post('/update', (req, res) => {
  try {
    ...
    if (temp === undefined || gas === undefined) return res.status(400).json({ ok:false, error:'Missing: temp, gas' });
    const tempVal = parseFloat(Number(temp).toFixed(2));
    const gasVal  = Math.max(0, parseInt(gas, 10));
    if (isNaN(tempVal) || isNaN(gasVal)) return res.status(400).json({ ok:false, error:'temp/gas must be numbers' });

    const parsed = { temp:tempVal, gas:gasVal, fan:toBool(fan), alarm:toBool(alarm), mode:controlState.mode, updatedAt:new Date().toISOString() };
    sensorData = parsed;
    totalUpdates++;
    pushToBuffer(history, { ...parsed, ts:Date.now() }, CONFIG.MAX_HISTORY);
    checkAndLogAlert(parsed);

    console.log(`📡 #${totalUpdates} temp=${parsed.temp}°C gas=${parsed.gas}ppm fan=${parsed.fan} alarm=${parsed.alarm}`);
...
// ── Frontend → Send control ──
app.post('/control', (req, res) => {
  try {
    const { mode, fan, mute } = req.body || {};
    if (mode !== undefined) {
      if (!['auto','manual'].includes(mode)) return res.status(400).json({ ok:false, error:'mode must be auto or manual' });
    ...
    console.log(`🎮 Control → mode=${controlState.mode} fan=${controlState.fan} mute=${controlState.mute}`);
...
app.get('/history', (req, res) => {
  ...
  res.json({ count:recent.length, limit, data:recent });
});
...
app.listen(PORT, '0.0.0.0', () => {
  console.log('╔══════════════════════════════════════════╗');
  console.log('║   AEGIS SENTINEL BACKEND  v4.0.0         ║');
  console.log(`║   Port   : ${String(PORT).padEnd(30)}║`);
  console.log(`║   Started: ${new Date().toISOString().slice(0,19).padEnd(30)}║`);
  console.log('╚══════════════════════════════════════════╝');
});
